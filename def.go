...
type ImageInfoType struct {
	...
	bufferDir string // Directory to store temp file buffers in.
}
...
func (info *ImageInfoType) GobEncode() (buf []byte, err error) {
	fields := []interface{}{
		info.data.Bytes(), info.smask, info.n, info.w, info.h, info.cs,
...
func (info *ImageInfoType) GobDecode(buf []byte) (err error) {
	var rawData []byte
	fields := []interface{}{
		&rawData, &info.smask, &info.n, &info.w, &info.h,
	...
	if info.data == nil {
		var b buffer
		if info.bufferDir != "" {
			b, err = newTempFileBuffer(info.bufferDir)
			if err != nil {
				return
			}
		} else {
	...
	info.data.ReadFrom(bytes.NewReader(rawData))
...
type Pdf interface {
	...
	Close()
	ClosePath()
	CreateTemplateCustom(corner PointType, size SizeType, fn func(*Tpl)) (Template, error)
	CreateTemplate(fn func(*Tpl)) (Template, error)
...
type Fpdf struct {
	...
	buffer           buffer                     // buffer for the final PDF
	pages            []buffer                   // slice[page] of page content; 1-based
